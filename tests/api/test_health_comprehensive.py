...
class TestCheckMinioHealth:
    ...
    def test_check_minio_health_invalid_endpoint(self):
        ...
        with patch("src.api.health.settings.environment", "production"):
            with patch("src.api.health.settings.minio_endpoint", ""):
                with patch("src.api.health.settings.minio_secure", False):
                    result = check_minio_health()
                    assert result == HealthStatus.UNHEALTHY
...
class TestReadinessCheckEndpoint:
    ...
    async def test_readiness_check_all_healthy(self):
        ...
        with patch("src.api.health.check_postgres_health") as mock_pg:
            with patch("src.api.health.check_redis_health") as mock_redis:
                with patch("src.api.health.check_minio_health") as mock_minio:
                    with patch("src.api.health.bind_agent_context"):
