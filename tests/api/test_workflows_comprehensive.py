...
class TestStartWorkflow:
    ...
    async def test_start_workflow_success(self):
        ...
        request = WorkflowStartRequest(
            user_request="Create a new feature for the system",
            priority=5
        )
        user = {"sub": "user123", "roles": ["admin"]}
...
class TestApproveWorkflow:
    ...
    async def test_approve_workflow_success(self):
        ...
        mock_status.model_copy = MagicMock(return_value=MagicMock(
...
class TestWorkflowIntegration:
    ...
    async def test_workflow_lifecycle(self):
        ...
        with patch("src.api.workflows.bind_workflow_context"):
            with patch("src.api.workflows.bind_agent_context"):
                approve_response = await approve_workflow(workflow_id, approve_request, user)
    ...
    async def test_multiple_workflows(self):
        ...
        for i in range(3):
            request = WorkflowStartRequest(
                user_request=f"Create feature number {i} for the system",
                priority=5
            )
